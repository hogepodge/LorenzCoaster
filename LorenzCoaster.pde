class Lorenz
{
  ...
  float m_xmult, m_ymult, m_zmult;
  
  int m_index;
  
  Lorenz( int historyLength )
  {
    m_x = new float[ historyLength ];
    m_y = new float[ historyLength ];
    m_z = new float[ historyLength ];
    
    m_x[0] = random( 0.05, 0.5 );
    m_y[0] = 0.0;
    m_z[0] = 0.0;
    
    m_h = 0.005;
    
    // Witht the following parametes the ranges for the 
    // variables are:
    ...
    m_xmult = float( width ) / (21.529194 - m_xmin);
    m_ymult = float( height ) / (29.454922 - m_ymin);
    m_zmult = float( height ) / 54.32006;
  ...
  {
    ++m_index;
    if ( m_index == m_x.length )
    {
      m_index = 1;
      m_x[0] = m_x[ m_x.length-1 ];
      m_y[0] = m_y[ m_x.length-1 ];
      m_z[0] = m_z[ m_x.length-1 ];
    }
    
    m_x[ m_index ] = m_x[ m_index-1 ] + m_h * m_a * ( m_y[ m_index-1 ] - m_x[ m_index - 1 ] );
    m_y[ m_index ] = m_y[ m_index-1 ] + m_h  * ( m_x[ m_index - 1 ] * ( m_b - m_z[ m_index - 1 ] ) - m_y[ m_index - 1 ] );
    m_z[ m_index ] = m_z[ m_index-1 ] + m_h  * ( m_x[ m_index - 1 ] * m_y[ m_index - 1 ] - m_c * m_z[ m_index - 1 ] );
  }
  
  float x( int i )
  {
    i = m_index - i;
    if ( i < 0 ) i = m_x.length + i;
    return (( m_x[ i ] - m_xmin ) * m_xmult);
  }
  ...
  {
    int step =2;
    for ( int i = 0; i < m_x.length-step; i += step )
    {
      //point( x(i), y(i), z(i) );
      stroke( 255*(m_x.length-i) / m_x.length );
      line( x(i), y(i), z(i), x(i+step), y(i+step), z(i+step));
    }
  }
  ...
  {
    float x, y, z;
    for ( int i = 0; i < m_x.length-1; i += 3 )
    {
      x = this.x(i);
      y = this.y(i);
      z = this.z(i);
...
void setBg( PImage img, color col )
{
...
void draw()
{
  ...
  dx = lorenz.dx(); dy = lorenz.dy(); dz = lorenz.dz();
  norm = 1/ (dist( 0, 0, 0, dx, dy, dz ) );
  dx *= norm;
  dy *= norm;
  dz *= norm;
  float adjust = width/8.0;
  
  coasterCam.jump( lorenz.x(0) - adjust*dx, 
                   lorenz.y(0) - adjust*dy-adjust, 
                   lorenz.z(0) - adjust*dz );
  ...
  fill( 20 );
  beginShape( QUADS );
    vertex(0, 0, -1); vertex(width, 0, -1); vertex(width, height, -1); vertex(0, height, -1);
    vertex(0, height+1, 0); vertex(width, height+1, 0); vertex(width, height+1, height); vertex(0, height+1, height);
    vertex(-1, 0, 0); vertex(-1, height, 0); vertex(-1, height, height); vertex(-1, 0, height);
  endShape();
  stroke( 50 );
  lorenz.renderPlanes();
...
public class Camera
{
   ...
   public Camera()
   {
      this(height * 0.5f / tan(DEFAULT_FOV * 0.5f));
   }

   // Create a camera that sits on the z axis with a specified shot length
   public Camera(float   aShotLength)
   {
      this(0, 0, aShotLength);
   }

   // Create a camera at the specified location looking at the world origin
   public Camera(float   aCameraX, float aCameraY, float aCameraZ)
   {
      this(aCameraX, aCameraY, aCameraZ,
                  0,        0,        0);
   }
                 ...
                 float anUpX,    float anUpY,    float anUpZ)
   {
      this(aCameraX, aCameraY, aCameraZ,
           aTargetX, aTargetY, aTargetZ,
              anUpX,    anUpY,    anUpZ,
           DEFAULT_FOV, (float)(1f * width / height), 0, 0);
   ...
   public Camera(float anFoV, float anAspect, float aNearClip, float aFarClip)
   {
      this(0, 0, height * 0.5f / tan(anFoV * 0.5f),
   ...
   public Camera (float aNearClip, float aFarClip)
   {
      this(0, 0, height * 0.5f / tan(DEFAULT_FOV * 0.5f),
           aNearClip, aFarClip);
   }
                 ...
                 float anFoV, float aNearClip, float aFarClip)
   {
      this(aCameraX, aCameraY, aCameraZ,
           aTargetX, aTargetY, aTargetZ,
           anUpX,    anUpY,    anUpZ,
           anFoV, (float)(1f * width / height), aNearClip, aFarClip);
   }
                 ...
                 float anFoV, float anAspect, float aNearClip, float aFarClip)
   {
      ...
      theShotLength = mag(theDeltaX, theDeltaY, theDeltaZ);
   ...
   public void feed() {
      perspective(theFoV, theAspect, theNearClip, theFarClip);
      camera(theCameraX, theCameraY, theCameraZ,
   ...
   public void truck(float anAmount)
   {
      ...
      float magnitude = mag(directionX, directionY, directionZ);
   ...
   private void updateUp()
   {
      ...
      float magnitude = mag(theUpX, theUpY, theUpZ);
      ...
      if (theRoll != 0)
      {
         ...
         // Normalize this vector so that it can be scaled
         magnitude = mag(directionX, directionY, directionZ);
